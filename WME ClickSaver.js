...
(function() {
    ...
    let _settings = {};
    let _translation; // Translation object to use for all displayed text.
    ...
    function logDebug(message) {
        console.debug(`${SCRIPT_NAME}:`, message);
    }
    ...
    function loadSettingsFromStorage() {
        ...
        const loadedSettingsExt = JSON.parse(localStorage.getItem(EXTERNAL_SETTINGS_NAME)) || {};
        EXTERNAL_SETTINGS.toggleTwoWaySegDrawingShortcut = loadedSettingsExt.toggleTwoWaySegDrawingShortcut || '';
    }
    ...
    function saveSettingsToStorage() {
        if (localStorage) {
            ...
            // The "external" settings here were leftover from old code that used sandboxing.
            // TODO: combine "external settings" (no longer needed) with the original settings localStorage item.
            let keys = '';
                ...
                if (!isDisabled && ((roadType.category === 'pedestrian' && isPed) || (roadType.category !== 'pedestrian' && !isPed))) {
                    ...
                    $div.append(
                        $('<div>', {
                            ...
                            .text(_translation.roadTypeButtons[roadTypeKey].text)
            ...
            $('div.address-edit').prev('wz-label').append(
                $('<a>', {
                    ...
                    style: 'float: right;text-transform: none;'
                        ...
                        + 'font-weight: normal;'
                }).text(_translation.addAltCityButtonText).click(onAddAltCityButtonClick)
            );
        }
    }
    ...
    function addSwapPedestrianButton(displayMode) { // Added displayMode argument to identify compact vs. regular mode.
        ...
        if (selectedFeatures.length === 1 && selectedFeatures[0].model.type === 'segment') {
            ...
            $button.append('<i class="fa fa-blind fa-lg"></i><i class="fa fa-arrows-h fa-lg" style="color:#e84545"></i><i class="fa fa-car fa-lg"></i>')
                .attr({
                    title: _translation.prefs.showSwapDrivingWalkingButton_Title
                });
            ...
            $('#csBtnSwapPedestrianRoadType').click(() => {
                if (_settings.warnOnPedestrianTypeSwap) {
                    ...
                    if (!confirm(_translation.swapSegmentTypeWarning)) {
                        return;
                    }
                }
                ...
                if (segment.model.hasPaths()) {
                    WazeWrap.Alerts.error('Clicksaver', _translation.swapSegmentTypeError_Paths);
                    return;
                }
    ...
    function initUserPanel() {
        ...
        $roadTypesDiv.append(
            createSettingsCheckbox('csUseOldRoadColorsCheckBox', 'useOldRoadColors', _translation.prefs.useOldRoadColors)
        );
        Object.keys(ROAD_TYPES).forEach(roadTypeAbbr => {
            const roadType = ROAD_TYPES[roadTypeAbbr];
            ...
            if (['PLR', 'PR', 'RR', 'PB', 'OR'].includes(roadTypeAbbr)) { // added RR & PB by jm6087
                $roadTypesDiv.append(
                    ...
                    createSettingsCheckbox(`csClearNew${roadTypeAbbr}CheckBox`, `setNew${roadTypeAbbr}StreetToNone`,
                        _translation.prefs.setStreetCityToNone, _translation.prefs.setStreetCityToNone_Title,
                    ...
                    createSettingsCheckbox(`csSetNew${roadTypeAbbr}CityCheckBox`, `setNew${roadTypeAbbr}City`,
                        _translation.prefs.setCityToConnectedSegCity, '',
                        { paddingLeft: '30px', marginRight: '4px' }, { fontStyle: 'italic' })
                );
            }
        });

        const $tab = $('<li>', { title: SCRIPT_NAME }).append(
        ...
        $panel.append(
            ...
            $('<div>', { style: 'margin-top:20px;font-size:10px;color:#999999;' }).append(
                $('<div>').text(`version ${SCRIPT_VERSION}${SCRIPT_NAME.toLowerCase().includes('beta') ? ' beta' : ''}`),
                $('<div>').append(
                    $('<a>', { href: FORUM_URL, target: '__blank' }).text(_translation.prefs.discussionForumLinkText)
                )
            )
        );
    ...
    function init() {
        ...
        Object.keys(ROAD_TYPES).forEach(rtName => {
            ROAD_TYPES[rtName].text = _translation.roadTypeButtons[rtName].text;
        });

        document.addEventListener('paste', onPaste);

        // Save all settings when closing window.
        $(window).on('beforeunload', () => saveSettingsToStorage());
    ...
    // This is the default English translation. It should be used
    // if the call to retrieve the translation spreadsheet fails.
    function getDefaultTranslation() {
        return {
            roadTypeButtons: {
                St: { text: 'St' }, // Street
                PS: { text: 'PS' }, // Primary Street
                mH: { text: 'mH' }, // Minor Highway
                MH: { text: 'MH' }, // Major Highway
                Fw: { text: 'Fw' }, // Freeway
                Rmp: { text: 'Rmp' }, // Ramp
                OR: { text: 'OR' }, // Off-road / Not maintained
                PLR: { text: 'PLR' }, // Parking Lot Road
                PR: { text: 'PR' }, // Private Road
                Fer: { text: 'Fer' }, // Ferry
                WT: { text: 'WT' }, // Walking Trail
                PB: { text: 'PB' }, // Pedestrian Boardwalk
                Sw: { text: 'Sw' }, // Stairway
                RR: { text: 'RR' }, // Railroad
                RT: { text: 'RT' }, // Runway/Taxiway
                Pw: { text: 'Pw' } // Passageway
            },
            prefs: {
    ...
    function loadTranslations() {
        ...
        return new Promise(resolve => {
            // This call retrieves the data from the translations spreadsheet and then starts
            // the main part of the code. If the spreadsheet call fails, the default English
            // translation is used.
            $.getJSON(`${TRANSLATIONS_URL}?${DEC(API_KEY)}`).then(res => {
                allTranslations = convertTranslationsArrayToObject(res.values);
                logDebug('Translations loaded');
            }).fail(() => {
                logError('Error loading translations spreadsheet. Using default translation (English).');
            }).always(() => {
                // Be sure to load translation before calling init().
                if (allTranslations) {
                    let locale = I18n.currentLocale().toLowerCase();
                    if (!allTranslations.hasOwnProperty(locale)) {
                        locale = 'en';
                    }
                    _translation = allTranslations[locale];
                } else {
